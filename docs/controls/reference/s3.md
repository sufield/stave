...
### CTL.S3.ACCESSLOG.DESTINATION.001
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** cis_aws_v3.0: 2.1.4; fedramp_moderate: AC-3; gdpr: Art.32; iso_27001_2022: A.8.3; nist_800_53_r5: AC-3; nist_csf_2.0: PR.PS; owasp_subtractive: S08; soc2: CC6.1; subtractive_tier: deletion;
...
**S3 Access Point Owns Data in Another Account's Bucket**
...
S3 access point's owning account differs from its bucket's account. This is the delegation inversion — the AP itself is the cross-account foothold. The AP owner controls the AP policy and can grant access to the bucket's data through a surface the bucket owner cannot see in their bucket policy. The AP owner effectively has a policy-controlled window into another account's bucket.

**Remediation:** Review whether the cross-account AP is intentional. If it is, ensure the bucket policy restricts what the AP can access (S3 evaluates both the AP policy and bucket policy). If the AP is not required, delete it. The bucket owner should use s3:AccessPointAccount condition keys to control which accounts can create APs targeting their buckets.
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** hipaa: 164.312(a)(1); owasp_subtractive: S08; soc2: CC6.1; subtractive_tier: deletion;
...
**S3 Access Point Policy Grants Access to External Account**
...
S3 access point resource policy contains a statement granting access to a principal outside the AP's owning account. Access points are the second mechanism (beside bucket policies) for controlling S3 data access. An external principal grant in the AP policy is a cross-account data access path that bucket-policy reviews miss.
Delegation-conditioned self-account statements (the OAC-DELEG-FP pattern: self-account principal with aws:SourceArn or aws:SourceAccount condition) are NOT external grants. They are the owning account authorizing its own service to use the AP.

**Remediation:** Review the access point policy with aws s3control get-access-point-policy. If the cross-account grant is not required, remove it. If required, scope to specific actions and prefixes, and ensure the external account is within the organization's trust boundary.
...
### CTL.S3.BUCKET.PREDICTABLENAME.001
...
### CTL.S3.ENCRYPT.DENY.UNENCRYPTED.001

**Bucket Policy Does Not Deny Unencrypted Uploads**

- **Severity:** medium
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** cis_aws_v3.0: 2.1.1; nist_800_53_r5: SC-28; soc2: CC6.1;

S3 bucket has default encryption enabled but the bucket policy does not deny PutObject requests that omit server-side encryption. Default encryption applies only when the caller does not specify encryption. An explicit ServerSideEncryption: none in the request bypasses the default. Without a Deny condition on s3:x-amz-server-side-encryption, default encryption is a suggestion, not a control.

**Remediation:** Add a Deny statement to the bucket policy that rejects PutObject requests missing server-side encryption.

---

### CTL.S3.ENCRYPT.KEYPINNING.001

**Bucket Policy Does Not Enforce KMS Key ID on Uploads**
...
S3 bucket uses SSE-KMS default encryption but the bucket policy does not contain a Deny statement with a StringNotEquals condition on s3:x-amz-server-side-encryption-aws-kms-key-id. Default encryption sets a preference, not an enforcement — any caller with s3:PutObject can specify a different KMS key in the request, including one they created in their own account. New objects get encrypted with the attacker's key; old objects stay on the legitimate key. Same attack shape as CVE-2020-8259 (Nextcloud encryption key substitution).

**Remediation:** Add a Deny statement to the bucket policy that rejects PutObject requests specifying a different KMS key. This locks all uploads to the designated key.

---

### CTL.S3.ENCRYPT.KMS.ENCRYPT.BROAD.001

**KMS Key Policy Grants Broad Encrypt Access**
...
The KMS key used for S3 encryption grants kms:Encrypt or kms:GenerateDataKey to overly broad principals — *, account root without conditions, or Principal: {"AWS": "*"}. Broad encrypt access undermines key-pinning controls: even if the bucket policy pins uploads to this key, an attacker with broad encrypt access can use the pinned key directly rather than substituting their own. This is the write-side complement to CTL.S3.ENCRYPT.KMS.POLICY.001 which checks decrypt breadth.

**Remediation:** Scope the KMS key policy to restrict kms:Encrypt and kms:GenerateDataKey to specific principals. Use kms:ViaService condition to limit usage to s3.amazonaws.com. Remove grants to * or account root without conditions.
...
### CTL.S3.LIFECYCLE.MULTIPART.001
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** nist_800_53_r5: SC-5; soc2: CC6.1;
...
**S3 Bucket in Local Zone Without Replication**

- **Severity:** high
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** hipaa: 164.308(a)(7); nist_800_53_r5: CP-9, SC-28; pci_dss_v4.0: 9.4.1; soc2: A1.2;

S3 bucket is placed in a Local Zone without cross-region replication configured. Local Zones provide single-AZ storage optimized for latency, not durability — there is no built-in cross-AZ redundancy. A zone failure, hardware issue, or network partition causes data unavailability or loss. Data that requires durability guarantees (compliance, backup, regulated) must replicate to a standard-region bucket.

**Remediation:** Configure cross-region replication to a standard-region S3 bucket. The destination bucket provides the durability guarantees that Local Zone storage lacks.
...
**MRAP Policy Has Broad Non-Service-Principal Grant**
...
The Multi-Region Access Point resource policy grants access to a non-service principal without a restricting condition. This includes Principal "*" or {"AWS": "*"} with no Condition, or an external IAM principal ARN with no scoping condition (aws:PrincipalOrgID, aws:SourceVpc, aws:SourceArn, etc.). Service-principal statements (cloudfront.amazonaws.com) are excluded — those are the OAC controls' domain. The MRAP policy is evaluated independently of bucket policies, so a broad grant here creates an access path regardless of bucket policy restrictions.

**Remediation:** Remove the wildcard or unconditioned principal grant. If external access is required, scope the grant with a Condition on aws:PrincipalOrgID, aws:SourceVpc, or aws:PrincipalArn. For CloudFront access, use OAC with the service principal pattern instead of direct IAM grants.

---

### CTL.S3.MRAP.POLICY.MIGRATION.RESIDUE.001

**MRAP Policy Has Both OAC and Legacy Role Grants**
...
The MRAP policy contains both a CloudFront service-principal Allow (the OAC pattern) and a non-service-principal IAM role/user Allow over overlapping actions (s3:GetObject or s3:*). This indicates a Lambda@Edge-era grant left beside a newer OAC grant. Lambda@Edge origin request triggers are incompatible with SigV4a OAC (per the CloudFront developer guide), so the role grant is residual and can be removed. Informational — the extra grant does not create a new exposure, but it widens the attack surface unnecessarily.

**Remediation:** Remove the IAM role/user Allow statement that overlaps with the CloudFront OAC grant. The OAC service-principal path supersedes Lambda@Edge role-based access for the same actions.

---

### CTL.S3.MRAP.POLICY.OAC.FOREIGNACCOUNT.001

**MRAP CloudFront Condition References Foreign Account**
...
The Multi-Region Access Point policy grants the CloudFront service principal with an aws:SourceArn condition, but the distribution ARN's account differs from the MRAP-owning account. This creates a cross-account read path dressed as a service grant — an external CloudFront distribution can read the MRAP's data through a legitimate-looking OAC configuration. The evidence names both accounts.

**Remediation:** Update the aws:SourceArn condition value to reference a CloudFront distribution in the same account as the MRAP. If cross-account access is intentional, document the trust relationship and add aws:SourceAccount as a secondary condition. Remove the grant if the cross-account path is unintentional.

---

### CTL.S3.MRAP.POLICY.OAC.NOCONDITION.001

**MRAP CloudFront Grant Missing aws:SourceArn Condition**
...
The Multi-Region Access Point resource policy grants the cloudfront.amazonaws.com service principal without the documented aws:SourceArn restricting condition. Without the condition any CloudFront distribution in any account can read through this MRAP — a confused-deputy vector. The AWS developer guide prescribes StringEquals on aws:SourceArn with the owning distribution's ARN.

**Remediation:** Add a Condition block with StringEquals on aws:SourceArn pointing to the specific CloudFront distribution ARN (arn:aws:cloudfront::ACCOUNT:distribution/DIST_ID). Use the SigV4a signing protocol for the OAC origin configuration.
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** cis_aws_v1.4.0: 2.1.5; cis_aws_v3.0: 2.1.4; fedramp_moderate: AC-3; ffiec: ISH-4; gdpr: Art.32; iso_27001_2022: A.8.3; nist_800_53_r5: AC-3; nist_csf_2.0: PR.PS; owasp_subtractive: S08; pci_dss_v3.2.1: 1.3.6; pci_dss_v4.0: 2.2.1; soc2: CC6.1; subtractive_tier: deletion;
...
- **Type:** unsafe_state
- **Domain:** identity
- **Compliance:** fedramp_moderate: AC-3; hipaa: 164.312(a)(1); nist_800_53_r5: AC-3; owasp_subtractive: S02; soc2: CC6.1; subtractive_tier: deletion;
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** cis_aws_v1.4.0: 2.1.5; fedramp_moderate: AC-3; ffiec: ISH-4; gdpr: Art.32; hipaa: 164.312(a)(1); iso_27001_2022: A.8.3; nist_800_53_r5: AC-3; owasp_subtractive: S08; pci_dss_v3.2.1: 1.2.1; pci_dss_v4.0: 7.2.1; soc2: CC6.1; subtractive_tier: deletion;
...
- **Type:** unsafe_state
- **Domain:** exposure
- **Compliance:** owasp_subtractive: S08; subtractive_tier: deletion;
