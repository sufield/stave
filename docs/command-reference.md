...
<!-- GENERATED by internal/tools/gencommanddocs — DO NOT EDIT.
     Regenerate with `make docs-commands`. Source of truth: the
     cobra command tree in the stave binary (prod edition). -->

All commands ship in the standard `stave` binary. No build tags are
...
| `config env list` | List supported STAVE_* environment variables |
