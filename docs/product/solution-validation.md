...
- **Version pinning is load-bearing.** Compose file names the exact Stave and Prowler versions. When either updates, a new tag releases the comparison at the new versions. Floating `latest` tags produce output drift that destroys the artifact's trust within weeks.
