...
- [`internal/airgap/policy.go`](../../internal/airgap/policy.go)
...
`text/template` is banned by default and only allowlisted for deterministic report rendering (`cmd/stave/cmd/report.go`). Enforcement artifacts (Terraform HCL, SCP JSON) are still generated by direct string formatting and struct marshaling.
...
- Restricted to a fixed set of predicate operators (see [Evaluation Semantics](../evaluation-semantics.md#predicate-operator-reference))
- Combined only via `any` (OR) and `all` (AND) logic
- Not extensible — no custom functions, no user-defined operators, no hooks

The operator set is defined in [`internal/domain/predicate_ops.go`](../../internal/domain/predicate_ops.go) and tested against drift.
...
This is enforced by a CI test (`TestNoBannedImportsInRuntime` in `internal/domain/`).
