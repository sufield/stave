...
The schema defining what properties are available for each asset type. Controls reference properties via dot notation in predicates. Extractors must emit conforming JSON. Documented per service in `docs/contract/`. See [contract/README.md](../explanation/contract/README.md).
