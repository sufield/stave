...
// `stave inspect acl`: production code here may import only pkg/stave,
...
//
// The ACL parse + assess + encode moved into stave.InspectACL
// (pkg/stave/acl.go). See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/inspect/acl/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the\n"+
