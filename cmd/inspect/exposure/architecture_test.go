...
// `stave inspect exposure`: production code here may import only pkg/stave,
...
//
// The exposure classification + bucket-access resolution + encode moved
// into stave.InspectExposure (pkg/stave/exposure.go). See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/inspect/exposure/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and\n"+
