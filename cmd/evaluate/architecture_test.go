...
// `stave evaluate`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Snapshot loading, profile evaluation, exception application, and report
// rendering moved into stave.EvaluateSnapshot (pkg/stave/evaluate.go); the
// command keeps only flag wiring, ui.ParseOutputFormat, the symlink-safe
// file sink (resolveOutput), the ExitCode classifier, and the exit-1
// failure signal. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/evaluate/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
