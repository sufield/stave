...
// `stave compare`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Both comparison modes — framework gap analysis and remediation impact —
// plus all three renderers moved into stave.CompareFrameworks /
// CompareRemediationImpact (pkg/stave/compare.go); the command keeps only
// flag wiring, input reading, and the file/stdout write. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/compare/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
