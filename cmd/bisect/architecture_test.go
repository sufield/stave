...
// `stave bisect`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Control loading, snapshot-archive loading, the bisect/scan engine, and
// both renderers moved into stave.BisectControl (pkg/stave/bisect.go); the
// command keeps only flag wiring, the stdout/stderr writes, and the exit-3
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/bisect/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
