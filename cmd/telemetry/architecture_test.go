...
// `stave telemetry`: production code here may import only pkg/stave,
...
//
// The assessment parse + filter + event mapping + NDJSON encode moved
// into stave.MapTelemetry (pkg/stave/telemetry.go). See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/telemetry/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the\n"+
