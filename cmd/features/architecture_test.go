...
// `stave features`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Registry discovery (control catalog, packs, frameworks, schemas, ATT&CK
// tactics), the scope-manifest load, and all three renderers moved into
// stave.RenderFeatures (pkg/stave/features.go); the command keeps only
// flag wiring and the pager. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/features/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
