...
// `stave search`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Catalog loading, the capability build + ranking, and both renderers
// moved into stave.RenderCatalogSearch (pkg/stave/search.go); the command
// keeps only flag wiring and input validation. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/search/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
