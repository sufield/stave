...
// `stave capabilities catalog`: production code here may import only
// pkg/stave, cmd/cmdutil, stdlib, third-party CLI deps, and the four
// exempt CLI helpers. _test.go files are exempt.
//
// Catalog loading, the capability build + filter, view resolution, and
// every renderer moved into stave.RenderCatalog (pkg/stave/catalog.go);
// the command keeps only flag wiring, the arg validator, and the pager.
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/catalog/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
