...
// `stave coverage`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Snapshot + control loading, the field-coverage analysis, and both
// renderers moved into stave.AnalyzeFieldCoverage
// (pkg/stave/fieldcoverage.go); the command keeps only flag wiring, the
// pager, the file/stdout sink, and the exit-3 SILENT_RISK signal. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/coverage/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
