...
// `stave expand`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Control loading, archetype resolution, filtering, snapshot scanning,
// and all four renderers moved into stave.ExpandList / ExpandArchetype
// (pkg/stave/expand.go); the command keeps only flag wiring, the pager,
// and input validation. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/expand/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
