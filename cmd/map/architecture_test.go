...
// `stave map`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Control loading, the coverage build, and all four renderers (table/
// json/navigator/markdown) moved into stave.MapAttackCoverage
// (pkg/stave/attack_map.go); the command keeps only flag wiring, the
// pager, the optional file read, and the file/stdout sink. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/map/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
