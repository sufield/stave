...
// `stave ci baseline`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Both subcommands' port wiring + use-case calls moved into
// stave.BaselineSave / BaselineCheck (pkg/stave/baseline.go); the command
// keeps only flag wiring, the output write, and the exit-3 new-findings
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/enforce/baseline/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
