...
// `stave ci gate`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// The policy parse, flag/clock/format resolution, the per-team gate filter,
// and the text/json render moved into stave.RunGate (pkg/stave/gate_run.go,
// over the existing stave.Gate); the command keeps only flag wiring, project
// config defaults, the stderr warnings, the output write, and the exit-3
// gate-fail signal. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/enforce/gate/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
