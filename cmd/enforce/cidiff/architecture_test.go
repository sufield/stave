...
// `stave ci diff`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// The evaluation comparison (loading both artifacts, diffing baseline vs
// current, computing new/resolved findings) moved into stave.CIDiff
// (pkg/stave/cidiff.go); the command keeps only flag wiring, the output
// write, and the exit-3 new-findings signal. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/enforce/cidiff/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
