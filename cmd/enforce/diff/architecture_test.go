...
// `stave snapshot diff`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// The snapshot loading, drift computation, filter parsing, identifier
// sanitization, and text/json rendering moved into stave.DiffObservationDrift
// (pkg/stave/internal/driftdiff); the command keeps only flag wiring, the
// progress span, and the quiet/output handling. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/enforce/diff/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
