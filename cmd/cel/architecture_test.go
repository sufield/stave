...
// `stave cel`: production code here may import only pkg/stave, cmd/cmdutil,
// stdlib, third-party CLI deps, and the four exempt CLI helpers. _test.go
// files are exempt.
//
// CEL evaluation (the celBridge adapter, asset parsing, and both
// renderers) moved into stave.EvalCEL (pkg/stave/cel.go); the command
// keeps only flag wiring and input reading. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/cel/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
