...
// `stave export-sir`: production code here may import only pkg/stave (and
// its cliapi sub-facade), cmd/cmdutil, stdlib, third-party CLI deps, and
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/exportsir/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
