...
// `stave contract`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Control/chain loading, the predicate-path index build, and both
// renderers moved into stave.ContractShowType / ContractList
// (pkg/stave/contract.go); the command keeps only flag wiring, the
// pager, and input validation. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/contract/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
