...
// `stave path`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Assessment parsing, chain loading, the attack-path graph build, and all
// three renderers moved into stave.BuildAttackPath
// (pkg/stave/attackpath.go); the command keeps only flag wiring, the
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/path/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
