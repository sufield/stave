...
// `stave doctor`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Running the environment checks and rendering the report moved into
// stave.RunDoctor (pkg/stave/doctor.go); the command keeps only flag
// wiring, format/working-directory resolution, and the exit-3 readiness
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/doctor/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
