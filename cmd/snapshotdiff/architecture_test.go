...
// `stave diff`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Both diff engines (catalog diff + snapshot diff), the control/bundle
// loading, and both renderers moved into stave.DiffCatalogs /
// DiffSnapshotBundles (pkg/stave/snapshotdiff.go); the command keeps only
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/snapshotdiff/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
