...
// `stave scorecard`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Assessment parsing, framework computation, and all three renderers
// (json/markdown/table) moved into stave.RenderScorecard
// (pkg/stave/scorecard.go); the command keeps only flag wiring and input
// reading. See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/scorecard/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
