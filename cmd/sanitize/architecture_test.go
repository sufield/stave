...
// `stave sanitize`: every Go file here may import only pkg/stave,
...
//
// Load + sanitize + marshal moved into stave.SanitizeSnapshot
// (pkg/stave/sanitize.go). See docs/architecture/pkg-stave-facade.md.
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/sanitize/ may import only pkg/stave, cmd/cmdutil, stdlib, and the four exempt CLI\n"+
			"helpers (see docs/architecture/pkg-stave-facade.md). Move new capability into pkg/stave.\n"+
