...
// `stave permissions` (nep): production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI helpers.
// _test.go files are exempt.
//
// The IAM net-effective-permission resolution (six policy layers) and the
// principal / resource / summary renderers moved into stave.ResolvePrincipal
// / ResolveResourceAccess / NepSummary (pkg/stave/internal/nepcmd); the
// commands keep only flag wiring, the output write, the resource stderr
// warnings, and the resource exit-1 security signal. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/nep/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
