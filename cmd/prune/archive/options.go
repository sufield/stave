...
// options holds the raw CLI flag values for the archive command.
type options struct {
	ObsDir     string
	ArchiveDir string
	OlderThan  string
...
func (o *options) BindFlags(cmd *cobra.Command) {
	f := cmd.Flags()
	f.StringVarP(&o.ObsDir, "observations", "o", o.ObsDir, "Path to active observation snapshots directory")
	f.StringVar(&o.ArchiveDir, "archive-dir", o.ArchiveDir, "Path to archive directory")
	f.StringVar(&o.OlderThan, "older-than", "", cliflags.WithDynamicDefaultHelp("Archive snapshots older than this age (e.g., 14d, 720h)"))
	...
	f.IntVar(&o.KeepMin, "keep-min", o.KeepMin, "Minimum number of snapshots to keep in active observations")
