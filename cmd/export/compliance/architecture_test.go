...
// `stave export compliance`: production code here may import only
// pkg/stave, cmd/cmdutil, stdlib, third-party CLI deps, and the four
// exempt CLI helpers. _test.go files are exempt.
//
// The whole load → evaluate → render pipeline (control + snapshot loading,
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/export/compliance/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
