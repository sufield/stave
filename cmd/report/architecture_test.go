...
// `stave report`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// The five-port report builder (artifact/chain/SLA/snapshot-bundle/
// control loaders), the build pipeline, and both renderers moved into
// stave.BuildReport (pkg/stave/report.go); the command keeps only flag
// wiring and the file/stdout sink. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/report/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
