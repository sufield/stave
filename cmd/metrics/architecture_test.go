...
// `stave metrics`: production code here may import only pkg/stave,
// cmd/cmdutil, stdlib, third-party CLI deps, and the four exempt CLI
// helpers. _test.go files are exempt.
//
// Loading the latest assessment from history, computing the posture
// score, grouping findings by team, and rendering the Prometheus body
// moved into stave.RenderMetrics (pkg/stave/metrics.go); the command
// keeps only flag wiring and file output. See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/metrics/ production code may import only pkg/stave, cmd/cmdutil, stdlib, and the four\n"+
