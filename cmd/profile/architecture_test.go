...
// `stave profile`: every Go file here may import only pkg/stave,
...
//
// The profile loading/validation moved into stave.ListEmbeddedProfiles
// and stave.LoadProfile (pkg/stave/profile.go). See
...
func TestArchitecture_FacadeOnly(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Errorf("cmd/profile/ may import only pkg/stave, cmd/cmdutil, stdlib, and the four exempt CLI\n"+
