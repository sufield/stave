...
type CompoundFinding struct {
	...
	Severity hipaa.Severity `json:"severity"`
...
type CompoundRule struct {
	...
	Matches func(results []hipaa.Result) bool
}
...
func Detect(rules []CompoundRule, results []hipaa.Result) []CompoundFinding {
...
func resultFailed(results []hipaa.Result, id string) bool {
...
func resultPassed(results []hipaa.Result, id string) bool {
	for _, r := range results {
		if r.ControlID == id {
			return r.Pass
		}
