...
// runGraphExportCypher runs stave graph export --format neo4j-cypher
// and returns the raw Cypher output.
func runGraphExportCypher(t *testing.T, bin, repoRoot, assessmentPath string) string {
	t.Helper()
	cmd := exec.Command(bin, "graph", "export",
		"--output", assessmentPath, "--format", "neo4j-cypher")
	cmd.Dir = repoRoot
	stdout, err := cmd.Output()
	if err != nil {
		var ee *exec.ExitError
		if errors.As(err, &ee) {
			t.Fatalf("graph export cypher failed (exit %d):\n%s", ee.ExitCode(), ee.Stderr)
		}
		t.Fatalf("graph export cypher failed: %v", err)
	}
...
// ---------------------------------------------------------------------------
// Experiment 5: Cypher export format
// ---------------------------------------------------------------------------

func TestExperiment05_CypherExport(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping: builds CLI binary")
	}
	...
	cypher := runGraphExportCypher(t, bin, repoRoot, assessment)

	// Verify it contains MERGE statements for nodes.
	if !contains(cypher, "MERGE (n:Finding") {
		t.Error("Cypher output missing Finding MERGE")
	}
	if !contains(cypher, "MERGE (n:Resource") {
		t.Error("Cypher output missing Resource MERGE")
	}
	if !contains(cypher, "MERGE (n:ThreatChain") {
		t.Error("Cypher output missing ThreatChain MERGE")
	}

	// Verify edge MERGE statements.
	if !contains(cypher, "[:TARGETS]") {
		t.Error("Cypher output missing TARGETS relationship")
	}
	if !contains(cypher, "[:MEMBER_OF]") {
		t.Error("Cypher output missing MEMBER_OF relationship")
	}

	// Verify header comment.
	if !contains(cypher, "Generated by stave graph export") {
		t.Error("Cypher output missing header comment")
	}
}
