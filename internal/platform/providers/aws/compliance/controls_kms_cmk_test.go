...
func TestControls001Strict(t *testing.T) {
	ctl := core.GetControlRegistry().Lookup("CONTROLS.001.STRICT")
	if ctl == nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			snap:     snap(encBucket("b", true, string(aws.AlgorithmAWSKMS), "arn:aws:kms:us-east-1:123:key/abc-123")),
			wantPass: true,
		},
		{
			name:        "SSE-KMS with AWS-managed key alias — fail",
			snap:        snap(encBucket("b", true, string(aws.AlgorithmAWSKMS), "alias/aws/s3")),
			wantPass:    false,
		...
		},
		{
			...
			snap:        snap(encBucket("b", true, string(aws.AlgorithmAWSKMS), "")),
			wantPass:    false,
...
func TestControls001Strict_RegisteredSeparately(t *testing.T) {
	base := core.GetControlRegistry().Lookup("CONTROLS.001")
	strict := core.GetControlRegistry().Lookup("CONTROLS.001.STRICT")
