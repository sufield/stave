...
## test-docs: Verify CLI commands in docs/ still parse and run.
##
## Extracts stave invocations from Markdown code blocks, builds the
## binary, and executes each command. Blocks annotated with
## # doctest:skip are reported but not run. Uses the integration
## build tag so regular `go test ./...` skips these.
test-docs: build
	$(GOTEST) -tags=integration -v -count=1 -timeout 5m ./internal/docsverify/...
...
## arch-graph: Regenerate the architecture dependency graph SVG.
arch-graph:
	go-arch-lint graph --project-path . --out docs-internal/architecture-graph.svg
...
gofixer:
	...
	@echo "4) Run new(expr) modernizer explicitly"
	$(GOCMD) fix -newexpr ./...
...
## validate-universals: Run the universal statement validation harness (27 universals × 2 fixtures)
validate-universals:
	@$(MAKE) build >&2
	@$(GOCMD) run ./internal/tools/validate-universals

## prove-universals: Direction 1 — ground observations into SMT-LIB, verify via Z3
prove-universals:
	@$(GOCMD) run ./internal/tools/prove-universals $(ARGS)
...
## validate-checklists: Verify structural and partition invariants on data/frameworks/*.yaml
validate-checklists:
	@$(GOCMD) run ./internal/tools/validate-checklists $(ARGS)

## validate-embed: Verify disk controls == compiled controls (catches b9c722e embed gaps)
validate-embed:
	@$(MAKE) build >&2
	@$(GOCMD) run ./internal/tools/validate-embed
