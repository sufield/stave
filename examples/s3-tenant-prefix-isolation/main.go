// Command s3-tenant-prefix-isolation demonstrates Stave's
// library API against the s3-tenant-prefix-isolation invariant
// — a shared multi-tenant S3 bucket whose presigned-URL signer
// fails to enforce per-tenant prefix isolation.
//
// CEL evaluation: does the unsafe state (signer with
// enforce_prefix=false OR allow_traversal=true) hold?
//
// The companion Z3 program at z3prove/ enumerates the concrete
// cross-tenant request the signer admits but the application
// never intended.
package main
...
const (
	invariantID stave.ControlID = "CTL.S3.TENANT.ISOLATION.001"
...
func main() {
	...
	if phase == "before" || phase == "both" {
		ok := runScenario(ctx, scenario{
			label:       "before (signer permissive)",
