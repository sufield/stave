#!/usr/bin/env bash
# Run every encoding-format test case in this directory.
# Each test case is a pair:
#   <name>.facts.jsonl    input fact set
#   <name>.expected.txt   expected human-readable output
#
# The test feeds the JSONL into format_facts.py with NO_COLOR=1
# (so the diff is golden-friendly) and compares to the expected
...
formatter="$explain_dir/format_facts.py"
