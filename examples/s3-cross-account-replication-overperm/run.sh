#!/usr/bin/env bash
# Iter 11 / s3-cross-account-replication-overperm — SMT-LIB
# file-boundary verification using the resource-policy
# Principal/Action projector added in PR 5. The bucket policy
# carries no Condition blocks, so PR 4's stringified-condition
# extraction produced zero discriminating facts; PR 5 lifts
# Statement.Principal and Statement.Action through
# resource_policy_principal / resource_policy_action.
#
# The query asserts the bucket grants either the source-account
# :root principal or the broad-read s3:Get*/s3:List* actions.
# Sat on writeup-config (both shapes are present), unsat on
# remediated-config (specific role only, replication actions
# only).
